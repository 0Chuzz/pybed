#ifndef lint
static char     rcsid[] =
"$Id: getline_.c,v 1.1 2000/05/04 11:11:48 pfw Exp $";
...
char           *getline_();		/* read a line of input */
...
static void	gl_kill_word();		/* delete left or right word */
...
/* get a character without echoing it to screen */
{
    ...
    /*    c = (ch <= 0)? -1 : ch; */
    c = (ch <= 0)? 0 : ch;   /* Changed by pfw -- Extended ASCII chars are now ignored */
#endif	/* __unix__ */
#ifdef MSDOS
...
/* set up variables and terminal */
{
	...
	gl_error("\n*** Error: getline_(): not interactive, use stdio.\n");
    gl_char_init();
    gl_init_done = 1;
}
...
/* adds the character c to the input buffer at current location */
{
	...
	gl_error("\n*** Error: getline_(): input buffer overflow\n");
...
static int gl_word_pos( direction )
int direction;
{
    int pos = gl_pos;
...
{
    gl_fixup(gl_prompt, -1, gl_word_pos( direction) );
}

static void 
gl_kill_word(direction)
int direction;
/* delete word forward or backward */
{
  int pos = gl_word_pos( direction );

  char *p = gl_killbuf;

  int l_pos = ( direction < 0 ? pos : gl_pos );
  int r_pos = ( direction < 0 ? gl_pos : pos );
  int i, len = r_pos - l_pos;
  ...
  *p = '\0';
  ...
  for ( i = r_pos; i < gl_cnt; i++ ) 
    gl_buf[i] = gl_buf[i+len];

  gl_fixup(gl_prompt, l_pos, l_pos );
}
...
char *buf;
{
    ...
    /* in case we call gl_histadd() before we call getline_() */
    if (gl_init_done < 0) {		/* -1 only on startup */
        hist_init();
        gl_init_done = 0;
    }
